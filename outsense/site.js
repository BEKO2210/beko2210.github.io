...
if(dial&&!reduced){const needle=dial.querySelector('.rose-needle');const reading=dial.querySelector('[data-bearing]');dial.addEventListener('pointermove',e=>{const r=dial.getBoundingClientRect();const angle=(Math.atan2(e.clientY-r.top-r.height/2,e.clientX-r.left-r.width/2)*180/Math.PI+90+360)%360;needle.style.transform=`rotate(${angle}deg)`;reading.textContent=`${String(Math.round(angle)).padStart(3,'0')}°`});dial.addEventListener('pointerleave',()=>{needle.style.transform='rotate(341deg)';reading.textContent='341°'})}
